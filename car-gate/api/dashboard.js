...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Cars (skip header rows that don't look like data)
    const cars = carPark
    ...
    // Filter data rows (skip non-date header rows)
    ...
    // Top drivers per car (top 3 per car index)
    const driversByCar = {};
    ...
    // Frequent guests (count entries per plate/name)
    const guestMap = {};
    guestRows.forEach(r => {
      if (r[5] !== 'เข้า') return;
    ...
    // Recent activity — last 30 member rows + last 15 guest rows, sorted desc, top 15
